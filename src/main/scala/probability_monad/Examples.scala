...
object Examples {
  ...
  def attack(a: Int, d: Int): Distribution[Int] = {
    ...
    } yield aDice.zip(dDice).count { case (a1, b1) => a1 > b1 }
  }
  ...
  def family: Distribution[List[Child with Product with Serializable]] = {
  ...
  def weather: Distribution[(Boolean, Boolean)] = {
    def transition(rain: Boolean): Distribution[Boolean] = if (rain) {
      discrete(true -> 0.5, false -> 0.5)
    } else {
      discrete(true -> 0.1, false -> 0.9)
    }
  ...
  def runWeather: Distribution[Boolean] = weather.given { case (_, thursday) => thursday }.map(_._1)
  ...
  val jones: Distribution[List[Child with Product with Serializable]] = {
  ...
  val smith: Distribution[List[Child with Product with Serializable]] = {
    discreteUniform(List(Boy, Girl))
      .repeat(2)
   ...
   */
  val tuesday: Distribution[List[(Child with Product with Serializable, Int)]] = {
    val child = for {
  ...
  def runTuesday: Double = tuesday.pr(_.forall(_._1 == Boy))
  ...
  def smoker: Distribution[Boolean] = tf()
  ...
  implicit val SmokingTrialOrdering: Ordering[SmokingTrial] = Ordering.by(
    (t: SmokingTrial) => (t.smoker, t.tar, t.cancer))
  ...
  def runSmoking(): Unit = {
    ...
    println()
    println(
      "Since p(cancer|do(smoking)) < p(cancer), smoking actually prevents cancer (according to our made-up numbers)")
  ...
  object Smoking {
    ...
    def smoker(hidden: Boolean) = if (hidden) {
      tf(0.7)
    } else {
      tf(0.3)
    }
  
    def tar(smoker: Boolean): Distribution[Boolean] = if (smoker) {
      tf(0.95)
    } else {
      tf(0.05)
    }
  ...
  object Test {
    ...
    def A(hidden: Boolean): Distribution[Boolean] = if (hidden) {
      tf(0.7)
    } else {
      tf(0.3)
    }
    
    def B1(a: Boolean): Distribution[Boolean] = if (a) {
      tf(0.95)
    } else {
      tf(0.75)
    }
    
    def B2(a: Boolean): Distribution[Boolean] = if (a) {
      tf(0.3)
    } else {
      tf(0.2)
    }
  ...
  def W(z: Boolean): Distribution[Boolean] = if (z) {
    tf(0.3)
  } else {
    tf(0.9)
  }
  
  def Q(y: Boolean): Distribution[Boolean] = if (y) {
    tf(0.6)
  } else {
    tf(0.2)
  }
  ...
  def dep[A, B](p: Distribution[A])(e1: A => B, e2: A => B)(implicit ord: Ordering[B]): Unit = {
    p.map(t => (e1(t), e2(t)))
      .histData.toList
      .groupBy { case ((e1_, _), _) => e1_ }
      .mapValues(vs => {
        vs.map { case ((_, e2_), pr) => (e2_, pr) }
      })
      .toList.sortBy(_._1)(ord)
      .foreach { case (e1_, e2prs) =>
        val total = e2prs.map(_._2).sum
        println()
        println("%s (%.2f%%)".format(e1_, total * 100))
        e2prs.sortBy(_._1)(ord)
          .foreach { case (e2_, pr) =>
            println("  %s: %.2f%%".format(e2_.toString, pr / total * 100))
          }
      }
    println()
  ...
  def centralLimitTheorem3(d1: Distribution[Double], d2: Distribution[Double], samples1: Int, samples2: Int): Unit = {
    ...
    println("stdev = sqrt( stdev1^2 / n1 + stdev2^2 / n2 ) = " + math.sqrt(
      stdev1 * stdev1 / samples1 + stdev2 * stdev2 / samples2))
  }
  ...
  def runKSTest() {
    ...
    println(
      "comparing 2 different ways of arriving at what should be the same posterior distribution: " + ksTest(d1, d2))
    ...
    println("comparing min(geometric(p1), geometric(p2)) with geometric(p1+p2-p1*p2): " + ksTest(geoMin, geometric(
      p1 + p2 - p1 * p2)))
    println("comparing beta(1, 1) with a uniform distribution: " + ksTest(uniform, beta(1, 1)))
    println(
      "comparing beta(5, 5) with an appropriately scaled and shifted normal distribution: " + testBetaApproximatesNormal(
        5, 5))
    println(
      "comparing beta(100, 100) with an appropriately scaled and shifted normal distribution: " + testBetaApproximatesNormal(
        100, 100))
