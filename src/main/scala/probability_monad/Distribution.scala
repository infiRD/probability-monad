...
abstract class Distribution[A] {
  ...
  def samplePar(n: Int = N): ParSeq[A] = (0 until N).par.map(_ => self.get)
  ...
  def bucketedHist(buckets: Int)(implicit ord: Ordering[A], toDouble: A <:< Double) {
    ...
    val min = data.head
    val max = data.last
    val (outerMin, outerMax, _, nBuckets) = findBucketWidth(toDouble(min), toDouble(max), buckets)
...
object Distribution {
  ...
  val uniform: Distribution[Double] = new Distribution[Double] {
   ...
   * @param d2 - distribution 1
   * @param n  - number of samples. Default = 100_000
   */
  def ksTest[A](d1: Distribution[A], d2: Distribution[A], n: Int = 100000)(implicit ord: Ordering[A]): Double = {
